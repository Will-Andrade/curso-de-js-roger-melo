...
const foodsLog = brazilianFoods.join(', ').replace(', B', ' e B');
const arraySize = brazilianFoods.length
const foodsInfo = `Até aqui, o array "brazilianFoods" possui ${arraySize} itens: ${foodsLog}.`;
...
const lastArrayItem = dogNames[dogNames.length - 1];
...
const firstPart = dogNames[0].slice(3);
const secondPart = dogNames[1].slice(2);
const thirdPart = dogNames[3].slice(2).replace('na', 'da');
const dessert = `${firstPart}${secondPart}${thirdPart}`;
