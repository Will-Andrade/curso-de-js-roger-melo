...
const my3FavoriteTVShows = ['X - Files', 'Breaking Bad', 'Spartacus'];
const tvShowMessage = my3FavoriteTVShows.join(', ').replace(', S', ' e S');
const sentence = `Minhas 3 séries favoritas são: ${tvShowMessage}`;
...
const typeofTVShow = typeof isRandomTVShowIncluded;
const typeSentence = `O tipo de dado que a const "isRandomTVShowIncluded" armazena é: ${typeofTVShow}.`;
...
const limit = 39;
const checkSize = String(typeSentence.length > limit).replace('t', 'T');

// console.log(`A string que a "typeSentence" armazena tem mais de ${limit} caracteres? ${checkSize}`);
...
const isLessThanOrEqualTo92 = agesSum <= 92;

// console.log(`A soma entre o 1º e o 3º item de "ages" é menor ou igual a 92. Essa afirmação é: ${isLessThanOrEqualTo92}.`);
